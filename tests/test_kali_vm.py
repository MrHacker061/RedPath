...
SSH_CONFIG = """Host kali-headless
  HostName 127.0.0.1
  User vagrant
  Port 2207
  IdentityFile C:/Users/test/private_key
  IdentitiesOnly yes
"""
...
class ParserTests(unittest.TestCase):
    ...
    def test_dynamic_ssh_config(self):
        config = parse_ssh_config(SSH_CONFIG)
        self.assertEqual(config.port, 2207)
        self.assertTrue(config.identity_file.is_absolute())

    def test_ssh_config_restrictions(self):
        with self.assertRaises(KaliVMError): parse_ssh_config(SSH_CONFIG.replace("127.0.0.1", "192.168.1.5"))
        with self.assertRaises(KaliVMError): parse_ssh_config(SSH_CONFIG.replace("IdentitiesOnly yes", "IdentitiesOnly no"))
...
class ManagerTests(unittest.TestCase):
    ...
    def manager(self, responses):
        ...
        return KaliVMManager(self.root, runner=runner), runner
